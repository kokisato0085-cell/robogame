...
### 武器バリエーション（確定）
発射体モデルの上に役割の異なる武器を用意し、「武器ごとに最適なアルゴリズムが変わる」ようにする。`WeaponSpec` に `pellets`（既定1）・`spreadDeg` を追加（拡散用）。

数値は小方針（カタログ data.ts の1か所のデータ）。動かしながら自由に調整する。弾サイズは観戦描画で武器ごとに変える。

| 武器 | power | range | cooldown | heat | projSpeed | pellets/spread | 弾 | w/電/slot |
|------|------:|------:|---------:|-----:|----------:|----------------|----|-----------|
| Cannon（基準） | 14 | 350 | 30 | 16 | 36 | 1 | 大 | 8/6/1 |
| Rapid SMG | 4 | 200 | 4 | 5 | 48 | 1 | 小 | 7/5/1 |
| Rail Sniper | 40 | 600 | 60 | 45 | 100 | 1 | 細長 | 14/12/2 |
| Laser | 9 | 380 | 6 | 26 | 130 | 1 | 細 | 9/10/1 |
| Scatter | 6 | 170 | 26 | 22 | 32 | 3 / 12° | 小×3 | 10/8/2 |

- **Scatter**：1発で pellets 発を扇状に発射（spreadDeg の範囲）。
- 1機が複数武器を積んだ場合、当面は**最初の武器のみ使用**（複数同時射撃は将来）。
- **条件追加 `lineOfSight`（clear/blocked）**：自機と敵を結ぶ線分が遮蔽物と交差すれば blocked。カバー越しの間合い管理に使える。
- **前方カバー再設置**：各スポーン前に小カバーを追加（A前(345,665,60,60)・B前(1195,875,60,60)・点対称）。中央レーンは開けたまま（膠着防止）。
- **将来武器（バックログ）**：誘導弾（追尾）／反射ガン（射線が遮られている＝敵がカバー裏の時、停車して壁反射の弾道を計算して当てる）／**2武器装備＋武器切替**（複数武器を積み、武器チャネルで「どの武器を撃つか」をアルゴリズムで使い分け。例：遠距離はSniper・近距離はSMG）。
