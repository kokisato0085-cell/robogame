...
- **初期配置**：対称。挑戦者(200,800)／相手(1400,800)。各スポーンの**前にカバー遮蔽物**を置き、**初期位置では攻撃が通らない**（開幕の撃ち合い・スポーンキル防止）。
- **遮蔽物**：固定レイアウトの矩形 **5個**（決定論のため毎戦同一）。弾を止める／移動は内部に入れず**縁で停止**（すり抜けない）。Replay に含めクライアントが描画。
  - レイアウト（表示units・x,y=左上・w,h／小方針として調整可）：A前 (330,740,50,170)・B前 (1220,740,50,170)・中央 (775,740,50,170)・上 (700,420,60,160)・下 (840,1060,60,160)
