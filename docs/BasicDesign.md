...
- **初期配置**：**斜め・点対称**（中心(800,800)）。挑戦者(200,650)／相手(1400,950)。各スポーン前にカバー遮蔽物を置き、**初期位置では攻撃が通らない**。斜め配置で approach に X・Y 成分が出る。
- **遮蔽物**：固定レイアウトの矩形 **5個**（点対称・毎戦同一）。弾を止める／移動は**壁ずり**（軸別判定でスライド・すり抜けない、FunctionalDesign S2-3）。Replay に含めクライアントが描画。
  - 具体座標は FunctionalDesign S2-1（小方針・調整可）。
